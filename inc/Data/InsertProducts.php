...
class InsertProducts extends BaseController
{
	...
	function insert_all($product_data)
	{
		...
		//$products_datax = json_decode($json, true); // Decode it into an array
		echo '<pre>';
		//var_dump($products_datax);
		echo '</pre>';
		//$this->hmu_set_attributes($product_data);
	...
	function insert_products($products)
	{
		...
		{
			return array_map(array($this, 'insert_product'), $products); // Run 'insert_product' function from above for each product
		}
	}
	...
	function insert_product_attributes($post_id, $available_attributes, $variations, $brand_attr)
	{
	...
	function insert_product_variations($post_id, $variations, $product_sku)
	{
		global $wpdb;
	...
	// ingore this
	function hmu_set_attributes($product_data)
	{
